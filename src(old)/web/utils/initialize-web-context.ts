...
import type { IconStore, Theme } from "src(old)/types/context";
import type { AsyncEntry } from "src(old)/types/entries";

export interface WebContextData
  extends Omit<WebContext, "type" | "iconStore" | "i18n"> {
...
export async function initializeWebContext(
  astro: AstroGlobal,
  data: WebContextData,
) {
  await i18next.init({
    ...
    resources: {
      [data.locale.code]: { translation: (await data.translations).data },
    },
  });
