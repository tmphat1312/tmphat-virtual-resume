...
function getCustomColors() {
  ...
  // Base color palettes used in the project.
  // Related CSS variables are located in `src/styles/colors/(dusk|accent)`.
  const baseColors = ["dusk", "accent"];

  // Colors used in the code for styling.
  // Related CSS variables are located in `src/styles/colors/elements`.
  const elementColors = [
    "button-bg",
    "button-bg-contrast",
    "web-heading-underline",
    "primary-dark",
    "primary",
    "primary-light",
    "primary-contrast",
  ...
  for (const color of baseColors) {
    result[color] = Object.fromEntries(
      shades.map((shade) => {
        return [shade, `hsl(var(--${color}-${shade}) / <alpha-value>)`];
      }),
    );
  }
